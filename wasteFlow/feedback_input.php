...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if a file was uploaded. This part is not in the original form but good for future reference.
    // For now, we'll assume there are no file uploads.
    // $image = null;
    // if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
    //     $image = file_get_contents($_FILES['image']['tmp_name']);
    // }

    // SQL query to insert a new feedback/complaint record
    $sql = "INSERT INTO feedbacks (user_id, type, subject, details, priority) 
            VALUES ('$userId', '$type', '$subject', '$details', '$priority')";

    if ($conn->query($sql) === TRUE) {
        // Redirect to the output page after successful submission
...
<body class="wf-body">
    ...
    <main class="wf-main">
        <section class="wf-section">
            ...
            <!-- The form method is changed to POST to securely send data -->
            <!-- The form action is changed to this file (feedback_input.php) to process the data -->
